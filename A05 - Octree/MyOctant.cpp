...
MyOctant::MyOctant(uint a_MaxLevel, uint a_IdealEntity)
{
	...
	{
		//Get the entity, give it rigidbody
...
bool MyOctant::IsColliding(uint a_index)
{
	...
	//Only have to check AABB since nothing is rotating/scaling
	//Get vectors in global space
	MyEntity* pEntity = m_pEntityMngr->GetEntity(a_index);
	MyRigidBody* pRigidBody = pEntity->GetRigidBody();
...
void MyOctant::Subdivide(void)
{
	...
	for (uint i = 0; i < 8; i++)
	{
		...
		m_pChild[i]->m_pParent = m_pParent;
...
//Traverse whole tree, find end (no children), go back up to parent and kill children
//Do so recursively until back to original node
...
{
	//Start with 0 children and octant size of 0
	m_uChildren = 0;
	m_fSize = 0.0f;

	//Start ID and Level at 0
	m_uID = m_uOctantCount;
	m_uLevel = 0;
