...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	*/
	...
	vector4 t_v4 = vector4(vector4(a_pOther->GetCenterGlobal(), 1.0f) - (m_m4ToWorld * vector4(m_v3CenterL, 1.0f)));

	t_v4 = vector4(glm::dot(t_v4, m_m4ToWorld * vector4(AXIS_X, 0.0f)), glm::dot(t_v4, m_m4ToWorld * vector4(AXIS_Y, 0.0f)), glm::dot(t_v4, m_m4ToWorld * vector4(AXIS_Z, 0.0f)), 0.0f);
	vector3 t = vector3(t_v4.x, t_v4.y, t_v4.z);

	for (uint i = 0; i < 3; i++) {
		for (uint j = 0; j < 3; j++) {
			AbsR[i][j] = glm::abs(R[i][j]) + 0.00000001f;
		}
	}
	...
	for (int i = 0; i < 3; i++) {
		...
		if (glm::abs(t[i]) > ra + rb) result = true;
	...
	for (int i = 0; i < 3; i++) {
		...
		if (glm::abs(t[0] * R[0][i] + t[1] * R[1][i] + t[2] * R[2][i]) > ra + rb) result = true;
