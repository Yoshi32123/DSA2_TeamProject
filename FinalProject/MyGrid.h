...
namespace Simplex
{
	...
	class MyGrid
	{
		...
		std::vector<MyGrid*> m_lChild; //list of nodes that contain objects (this will be applied to root only)
	...
	public:
		...
		OUTPUT: class object
		*/
		MyGrid(uint a_nMaxLevel = 2, uint a_nIdealEntityCount = 5);
		/*
		USAGE: Constructor
		ARGUMENTS:
		...
		*/
		...
		ARGUMENTS:
		- MyGrid& other -> object to swap content from
		OUTPUT: ---
		*/
		...
		*/
		...
		/*
		USAGE: Displays the MyGrid volume specified by index including the objects underneath
		ARGUMENTS:
		- uint a_nIndex -> MyGrid to be displayed.
		- vector3 a_v3Color = REYELLOW -> Color of the volume to display.
		OUTPUT: ---
		*/
		void Display(uint a_nIndex, vector3 a_v3Color = C_YELLOW);
		/*
		USAGE: Displays the MyGrid volume in the color specified
		ARGUMENTS:
		- vector3 a_v3Color = REYELLOW -> Color of the volume to display.
		OUTPUT: ---
		*/
		...
		*/
		...
		OUTPUT: MyGrid object (child in index)
		*/
		MyGrid* GetChild(uint a_nChild);
		/*
		USAGE: returns the parent of the octant
		ARGUMENTS: ---
		OUTPUT: MyGrid object (parent)
		*/
		MyGrid* GetParent(void);
		/*
		USAGE: Asks the MyGrid if it does not contain any children (its a leaf)
		ARGUMENTS: ---
		OUTPUT: It contains no children
		*/
		bool IsLeaf(void);
		/*
		USAGE: Asks the MyGrid if it contains more than this many Bounding Objects
		ARGUMENTS:
